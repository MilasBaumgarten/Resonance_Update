...
namespace CMGTools {
	...
	public class SimulatePhysicsWindow : EditorWindow {
		...
		private void OnGUI() {
			...
			switch (currentState) {
				...
				case state.explosion:
					...
					if (pointObject != null && PrefabUtility.GetPrefabAssetType(pointObject) == PrefabAssetType.Regular) {
						pointObject = null;
					}
					break;
				case state.cannon:
		...
		public void PrepareSim() {
			...
			List<Transform> transforms = new List<Transform>(Selection.GetTransforms(SelectionMode.Editable));
		...
		private void OnSelectionChange() {
			...
			if (currentState != state.cannon) {
				foreach (Transform transform in Selection.GetTransforms(SelectionMode.Editable)) {
