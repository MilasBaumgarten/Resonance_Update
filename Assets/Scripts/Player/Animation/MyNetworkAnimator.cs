...
public class MyNetworkAnimator : NetworkBehaviour {
	...
	protected void Update() {
		...
		//Send parameters
		ChangeParametersServerRpc(memStream.ToArray(), dirties);
	}
	...
	[ServerRpc]
	void ChangeParametersServerRpc(byte[] data, int size) {
		ChangeParametersClientRpc(data, size);
	}
	...
	/// <param name="size">Number of AnimDatas</param>
	[ClientRpc]
	public void ChangeParametersClientRpc(byte[] data, int size) {
		if (IsLocalPlayer) {
			return;
		}
